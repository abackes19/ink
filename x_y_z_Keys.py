...
while key != ord('z'): #end loop if 'z' is hit
    key = screen.getch()
    screen.clear()

    if x <= 0.0: #varaibles can't go out of range
        x = 0.0
    ...
    if round(y, 1) == 0.0: #so y = 0.0 doesn't read "-0.0"
        y = 0.0
    if round(z, 1) == 0.0: #so z = 0.0 doesn't read "-0.0"
        z = 0.0
    ...
    if key != curses.ERR: #read key press
        if key == ord('w'): #increase y value
            ...
            if test(x, y, z) == False:
                x = x + 0.1 * speed
        elif key == ord('d'): #increase z value
            ...
            if test(x, y, z) == False:
                z = z + 0.1 * speed
        elif key == ord('2'): #increase speed
            screen.addstr(2, 57, '2 key', curses.color_pair(2))
            speed = int(speed + 1)
            if speed >= 4:
                speed = 4
        elif key == ord('1'): #decrease speed
            screen.addstr(2, 57, '1 key', curses.color_pair(2))
            speed = int(speed - 1)
            if speed <= 1:
                speed = 1
        elif key == ord('z'): #end the program and reformat the terminal
            curses.endwin()
