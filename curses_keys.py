...
x = 10.0; y = 10.0
screen = curses.initscr(); curses.noecho(); curses.halfdelay(3); screen.keypad(True); screen = curses.initscr() #set up curses file
...
while key != ord('q'): #to end loop if 'q' is hit
    ...
    screen.addstr(4, 0, 'Elbow motor value:'); screen.addstr(5, 0, 'Shoulder motor value:')
    ...
    screen.addstr(4, 19, str(input_elbow)); screen.addstr(5, 22, str(input_shoulder)) #print all values
    if (input_shoulder or input_elbow) < 400:
        screen.addstr(6, 0, 'Point outside of motor domain', curses.color_pair(1))
